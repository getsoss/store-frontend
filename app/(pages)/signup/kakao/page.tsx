...
export default function KakaoSignupPage() {
  ...
  const isValidPassword = (pw: string) =>
    pw.length >= 10 && /[a-zA-Z]/.test(pw) && /\d/.test(pw);
  ...
  return (
    <main className="mx-auto max-w-md p-4 mt-10">
      <h1 className="text-2xl font-bold mb-4">카카오 회원가입</h1>
      <form onSubmit={onSubmit} className="grid gap-3">
        <label className="grid gap-1">
          ...
          <input
            ...
            value={form.email}
            readOnly
            className="px-3 py-2 border border-gray-300 rounded-md bg-gray-100 focus:outline-none"
          />
        </label>
        <label className="grid gap-1">
          <span>이름</span>
